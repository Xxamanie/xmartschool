...
import { Student, Subject, User, UserRole, SchemeSubmission, Assessment, ResultData, School } from '../types';
...
// Helper to generate more student data for testing virtual scroll
const generateStudents = (count: number): Student[] => {
  ...
  for (let i = 8; i <= count; i++) {
    ...
    students.push({
      ...
      enrollmentDate: `202${Math.floor(Math.random() * 3 + 1)}-09-${Math.floor(Math.random() * 28 + 1).toString().padStart(2, '0')}`,
...
// Generate matching results for the extra students
...
export const DASHBOARD_STATS = [
  { label: 'Total Students', value: 1250, change: '+5%', positive: true },
  { label: 'Avg Attendance', value: '94.2%', change: '+1.2%', positive: true },
  { label: 'Upcoming Events', value: 4, change: 'This Week', positive: true },
  { label: 'Pending Reports', value: 12, change: '-2', positive: false },
];
